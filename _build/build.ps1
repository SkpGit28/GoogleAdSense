  ...
  <div class="cookie-consent">
    <div class="cookie-consent__inner">
      <div class="cookie-consent__text">
        This site uses cookies for basic analytics, and for advertising if you accept. Decline and no
        advertising or analytics cookies are set. <a href="${up}privacy.html">Read the privacy policy</a>.
      </div>
...
function Sidebar($current) {
  ...
  $dest = ""
  foreach ($o in ($all | Where-Object { $_.slug -ne $current } | Get-Random -Count ([Math]::Min(6, $all.Count - 1)) )) {
    ...
    <main id="main" class="article-content">
      ...
      <div class="methodology-box">
        <div class="methodology-box__header">
          <h4>How these hotels were researched</h4>
...
"@ + (Header "" "") + @"
  ...
  <main id="main" class="legal-page">
    ...
    <p>Most are Creative Commons licences that require attribution, which is what this page is
      ...
      <a href="mailto:contact@primehotelpicks.com">contact@primehotelpicks.com</a> and I will
...
foreach ($m in $all) {
  if (Test-Path "$root\articles\$($m.slug).html") {
    $urls += @{ loc = "$site/articles/$($m.slug).html"; pri = "0.8"; freq = "monthly" }
  }
}
$sm = '<?xml version="1.0" encoding="UTF-8"?>' + "`n" + '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">' + "`n"
foreach ($u in $urls) {
  $sm += "  <url>`n    <loc>$($u.loc)</loc>`n    <lastmod>$today</lastmod>`n    <changefreq>$($u.freq)</changefreq>`n    <priority>$($u.pri)</priority>`n  </url>`n"
}
$sm += '</urlset>' + "`n"
